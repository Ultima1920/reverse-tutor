...
if not os.environ.get("GEMINI_API_KEY"):
    st.warning(
        ...
        "Copy `.env.example` to `.env` locally (or add `GEMINI_API_KEY` under "
        "**Settings → Secrets** if this app is deployed on Streamlit Cloud), then restart the app.  \n"
...
with voice_cols[0]:
    if VOICE_INPUT_ENABLED:
        st.success("🎙️ Voice input ready — speak your answers in Explain / Error Hunt mode.")
    else:
        st.info("🎙️ Voice input unavailable — you can still type your answers.")
with voice_cols[1]:
    if VOICE_OUTPUT_ENABLED:
        st.success("🔊 Voice playback ready — the AI can read its responses aloud.")
    else:
        st.info("🔊 Voice playback unavailable — responses will show as text only.")
...
st.divider()
st.caption("Built with Streamlit · Google Gemini API (free tier) · SQLite · audio-recorder-streamlit · gTTS  |  No paid services required.")
